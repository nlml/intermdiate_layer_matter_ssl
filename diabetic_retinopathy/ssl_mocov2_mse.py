...
def main(args):
    data_module = BaselineDataModule(
        data_dir=args.data_path, num_workers=40, batch_size=args.batch_size
    )
    ...
    model = Moco_v2(datamodule=data_module, lamb_values=args.lamb_values)

    lamb_append_term = "-".join(np.array(args.lamb_values).astype(str))

    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(
            args.save_path, "mocov2_all_augs_dr_mse_" + lamb_append_term
        ),
...
def get_args():
    ...
    parser.add_argument(
        "--lamb_values",
        nargs="+",
        help="lamb for mse loss from each block of resnet50",
        type=float,
